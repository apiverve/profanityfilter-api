/**
 * Profanity Filter API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Profanity Filter API.
 * API Documentation: https://docs.apiverve.com/ref/profanityfilter
 */
 ...
 */
async function callProfanityFilterAPIWithErrorHandling(requestData) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callProfanityFilterAPIWithErrorHandling({
